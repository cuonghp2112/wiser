...
def score_labeling_functions_majority_vote(instances,  gold_label_key='tags',
                               treat_tie_as='O', span_level=True):
    tp, fp, fn = 0, 0, 0
    for instance in instances:
        maj_vote = _get_label_majority_vote(instance, treat_tie_as)
        if span_level:
            score = _score_sequence_span_level(
                maj_vote, instance[gold_label_key])
        else:
            score = _score_sequence_token_level(
                maj_vote, instance[gold_label_key])
        tp += score[0]
        fp += score[1]
        fn += score[2]
    ...
    index = ["Majority Vote"] if span_level else [
        "Majority Vote (Token Level)"]
...
def score_individual_labeling_functions(instances, gold_label_key='tags'):
    lf_scores = {}
    for instance in instances:
    ...
    for lf_name in lf_scores.keys():
        if lf_scores[lf_name][3] > 0:
            lf_scores[lf_name][3] = float(
                lf_scores[lf_name][3]) / lf_scores[lf_name][4]
    ...
    results = pd.DataFrame.from_dict(
        lf_scores, orient="index", columns=column_names)
    results = pd.DataFrame.sort_index(results)
    return results
...
def score_linking_functions(instances, gold_label_keys='tags'):
    ...
    # Collects results into a dataframe
    column_names = ["Entity Links", "Non-Entity Links",
                    "Incorrect Links", "Accuracy"]
    results = pd.DataFrame.from_dict(
        lf_scores, orient="index", columns=column_names)
    results = pd.DataFrame.sort_index(results)
    return results
...
def _score_sequence_span_level(predicted_labels, gold_labels):
    if len(predicted_labels) != len(gold_labels):
        raise ValueError(
            "Lengths of predicted_labels and gold_labels must match")
